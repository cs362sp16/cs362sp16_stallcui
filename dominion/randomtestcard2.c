...
int oracle(struct gameState g, int szhand, int* discards, int ncoins, int nbuys, int choice)
{
	int r;
	r = g.handCount[0];
	...
	else //estate should be added to hand
	{
//		printHand(0,&g);
		r = g.hand[0][6];
		cassert(r == estate, "estate added to hand");
	}
...
//	printf("# coins = %d\n",g.coins);
	//run oracle here
	oracle(g, szhand, discards, ncoins, nbuys, choice);
